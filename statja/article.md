...
The durational vocabulary of Western tonal music is built entirely from smooth numbers — integers of the form 2^a × 3^b. We ask whether this arithmetic constraint extends to the *count* of times a structural element recurs within a piece. Analysing 1,943 Bach works and a broader corpus of 4,447 files spanning Baroque and Classical composers, we find that motif occurrence counts are smooth numbers significantly more often than expected under a log-uniform null model (enrichment 2.23× for Bach). A shift test confirms the result is genuine: real counts are 1.33× denser at smooth positions than counts shifted by +1 (threshold ≥ 16), rising to 1.89× at threshold ≥ 36. Three Bach cases illustrate the pattern: WTC Prelude No. 7 in E♭ major (192 = 2^6·3 occurrences of a stepwise sixteenth-note figure), WTC Fugue No. 12 in F minor (96 = 2^5·3), and the BWV 944 Fugue in A minor (128 = 2^7). The effect holds across Baroque composers (Handel 1.77×, Telemann 1.97×, Corelli 1.89×, Scarlatti 1.99×, Buxtehude 1.66×, Frescobaldi 1.70×) and extends to the Classical period, with Mozart (2.05×) matching Baroque levels and Beethoven (1.66×) showing a distinct shift-test signature. The mechanism behind this alignment remains an open question.

*Keywords*: smooth numbers, motif analysis, corpus musicology, metric hierarchy, music perception
...
Western music theory has long recognized that the durational hierarchy of tonal music is organized around powers of two and three. Whole notes divide into halves, quarters, eighths, and sixteenths (powers of 2); dotted values and compound meters introduce factors of three; the result is that every standard note duration is a smooth number — an integer of the form 2^a × 3^b — when expressed as a fraction of a fixed unit. This constraint is so deeply embedded in notation that it goes unremarked: a "non-smooth" duration such as a fifth-note or a seventh-note would be not merely unusual but unnotatable in standard Western notation without resort to tuplets.
...
3. **Feature tuple.** Each melodic step is represented as a tuple (interval, duration, metric phase, onset, contiguity flag). The *metric phase* of a note is its position within the beat, discretized to the note's own duration as a unit (e.g. eighth notes in 4/4 time have two phases: 0 = on the beat, 1 = off the beat). *Contiguity* is false if a rest intervenes between two notes; patterns spanning rests are excluded. Two patterns that share the same (interval, duration) body but differ in start phase are treated as distinct patterns, since metric placement is a primary component of motivic identity in tonal music.
