...
KERN_DIR     = os.path.join(os.path.dirname(os.path.dirname(__file__)), "kern")
...
XML_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "musicxml")
...
def find_lilypond_files():
    ...
    """
    import re as _re
    xml_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'lilypond', 'musicxml')
...
def find_tobis_files():
    ...
    split_base = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'tobis-notenarchiv.de', 'split')
...
def _regen_tobis_splits(subdir='engl-suites'):
    ...
    base = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'tobis-notenarchiv.de')
...
def find_imslp_files():
    ...
    imslp_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'musicxml', 'imslp_bach')
...
from motif_analysis import *  # noqa: F401,F403
from motif_analysis import (
    _PITCH_CLASS, _DIATONIC_STEP, _MEI_NS, _XML_ID,
    _MOTIF_COLORS, _DUR_NAMES, _DIATONIC_NAMES, _BEAT_DUR_OVERRIDES,
    _search_motif, _search_attack_grid, _parse_dur, _dur_matches,
    _get_note_beat_positions, _find_motifs, _interval_seq,
    _voice_notes_from_mei, _merge_ornamental_slurs, _remove_unison_voices,
    _metric_phase, _mdl_score, _pattern_to_query, _interval_label,
    _dur_q_to_str, _dur_q_label, _to_midi, _to_quarters,
)
...
def _fix_beam_groups(content: str) -> str:
    ...
    self-contained (begin â†’ ... â†’ end) within a sequence of notes without backup
    ...
    for measure in root.iter(f'{pfx}measure'):
        ...
        # A <backup> resets the current segment index â€” notes after it are a new segment.
                    ...
                    for beam_el in note.findall(f'{pfx}beam'):
                        ...
                        if val == 'begin':
                            ...
                            if bnum not in open_beam:
                                # orphaned continue â€” treat as begin
                                beam_el.text = 'begin'
                                open_beam[bnum] = beam_el
                                changed = True
                            else:
...
def _fix_missing_tuplet_markers(content: str) -> str:
    ...
    regular eighth notes â€” causing lower-voice quarter notes to appear
...
def _fix_musicxml_voice_order(content: str) -> str:
    ...
    the highest average MIDI pitch (â†’ stems up) and voice 2 the next, etc.
...
def _renumber_measures_from_one(content: str) -> str:
    ...
    base_offset = min_num - 1  # normalise so first original number â†’ 1
    if _is_pickup:
        base_offset += 1       # first bar â†’ 0, first full bar â†’ 1
...
def _fix_missing_initial_clefs(content: str) -> str:
    ...
    Clef choice: avg MIDI < 58 â†’ bass (F/4), otherwise treble (G/2)."""
    import xml.etree.ElementTree as ET
    try:
...
# â”€â”€ TSD harmony labels â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Loaded once from TSD.txt: filename â†’ (beat_dur_q, [label, ...])
# label: 'T', 'S', or 'D'
...
_TSD_DATA: dict = _load_tsd_file()   # filename â†’ (beat_dur_q, ['T','S','D',...])
...
class FileBrowser(tk.Tk):
    def __init__(self):
        ...
        self._files        = find_generated_files() + find_lilypond_files() + find_kern_files(KERN_DIR) + find_tobis_files()
        self._current_path = None

        self._build_ui()
        self._populate_list()
        self.after(100, self._load_music21_files)

    def _load_music21_files(self):
        extra = find_music21_files()
        if extra:
            self._files += extra
            self._populate_list()
